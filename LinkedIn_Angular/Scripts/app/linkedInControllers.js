...
linkedInControllers.controller('linkedInController', [
    "$scope", function ($scope) {
        ...
        $scope.getLinkedInData = function() {
            IN.API.Raw("/people/~:(id,num-connections,picture-url,location,positions,educations,skills,phone-numbers,volunteer,email-address,summary,formatted-name)").result(onSuccess).error(onError);
        }
        ...
        function onSuccess(data) {
            ...
            $scope.professionalSummary = data.summary;
            $scope.phoneNumbers = data.phoneNumbers.values;
            $scope.email = data.emailAddress;
            $scope.positions = data.positions.values;
            $scope.educations = data.educations.values;
            $scope.$apply();
            console.log(data);
